...
class ResultTests: XCTestCase {
    ...
    func testPassthroughValueHandlerWithValue() {
        let input = Result.value(1)
        var accum: [Int] = []
        
        let transform = Result.transform(forPassthroughValueHandler: { (x: Int) in accum.append(x) })
        let output = transform(input)
    ...
    func testPassthroughValueHandlerWithError() {
        let input = Result<Int>.error(testError)
        var accum: [Int] = []
